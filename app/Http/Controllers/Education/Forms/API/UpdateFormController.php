...
use App\Services\Education\Forms\UpdateFormService;

use App\Http\Controllers\ActionController;

class UpdateFormController extends ActionController
{
    /**
    ...
    public function __invoke(Int $id)
    {
        ...
        try {
            ...
            // Execute Service(s)
            $svc = new UpdateFormService;
            $svc->setParameters([ 'form_id' => $id ]);
            $svc->setAttributes($attributes);
            $form = $svc->execute();

            return $this->json($form->toArray(), 200);
